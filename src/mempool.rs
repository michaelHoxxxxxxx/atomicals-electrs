...
use crate::{
    ...
    signals::ExitFlag,
    types::ScriptHash,
    atomicals::{AtomicalsManager, AtomicalOperation},
};
...
impl Mempool {
    pub fn new(metrics: &Metrics, atomicals: Arc<AtomicalsManager>) -> Self {
        Self {
    ...
    fn add_entry(&mut self, entry: Entry) {
        ...
        if let Ok(operations) = self.atomicals.get_operations(&entry.tx) {
            if !operations.is_empty() {
                self.apply_pending_operations(&entry.tx, operations.clone())?;
                self.pending_operations.write().insert(entry.txid, operations);
            }
        }
    }
    ...
    /// Apply pending operations to the mempool state.
    fn apply_pending_operations(&mut self, tx: &Transaction, operations: Vec<AtomicalOperation>) -> Result<()> {
        self.atomicals.apply_pending_operations(operations, tx, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)?.as_secs())?;
        Ok(())
    }

    /// Rollback pending operations from the mempool state.
    fn rollback_pending_operations(&mut self, txid: &Txid, operations: &[AtomicalOperation]) -> Result<()> {
        self.atomicals.rollback_pending_operations(txid, operations)?;
        Ok(())
    }

    /// Get all pending operations.
    pub fn get_pending_operations(&self) -> HashMap<Txid, Vec<AtomicalOperation>> {
